...
\title{Causal Geometric Structure in Neural Populations}
\subtitle{Resolving the linear-nonlinear divide in neural analysis tools\\[0.3em]
{\small Exploratory results on Neuropixels visual decision-making data}}
\author{Elliot Tower}
\date{2026}
\institute{}
...
\textbf{Part 1: Geometric dissociation}
\begin{enumerate}
  \item The question: does population shape matter?
  \item Linear vs.\ nonlinear tools disagree
  \item Causal interventions (IIA, subspace swap)
  \item Learned subspaces are stronger
  \item Validity checks and predictions
\end{enumerate}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Part 2: Going deeper}
\begin{enumerate}
  \setcounter{enumi}{5}
  \item The IIA vacuity problem (and why we survive)
  \item Expanded optogenetic validation
  \item Engagement disentanglement
  \item Multi-task generalization
  \item Neural manifold interpretation
  \item Running experiments and next steps
\end{enumerate}
\end{column}
\end{columns}
\end{frame}
...
\vspace{0.5em}
{\small Each dot is a pair of brain regions. The two tools rank them in \textbf{opposite order} ($\rho = -0.85$ biased CKA; $-0.45$ after debiasing---holds either way).}

\vspace{0.3em}
{\small Accounting for spread, disagreement shrinks to $-0.31$. Found in 5 sessions, confirmed across all 39. \textbf{Takeaway:} the tool you pick determines what you find.}
\end{column}
\end{columns}

\vspace{0.3em}
\begin{center}
{\small \textbf{Why?} The right panel colors dots by how ``spread out'' the neural activity is. Blue dots (compact) agree. Red dots (diffuse) disagree.}
\end{center}
\end{frame}
...
%% --- SLIDE: IBL replication ---
\begin{frame}{Cross-dataset replication: IBL data}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{The real test:} different mice, different labs, different rigs.
...
\textbf{What we test:}
\begin{itemize}
  \item Same subspace dimensions sufficient?
  \item Same region rankings emerge?
  \item Same CKA--Procrustes dissociation?
\end{itemize}

\vspace{0.5em}
\textbf{Status:} IBL replication confirms the dissociation ($\rho = -0.94$, $n = 11$ matched regions).
\end{column}
\end{columns}
\end{frame}
...
{\small more neurons $=$ more statistical power $=$ \textbf{higher} IIA (positive $\rho$)}
...
%% --- SLIDE: Prediction 2 ---
\begin{frame}{Prediction 2: dimensionality predicts causal strength}
...
{\small
\begin{tabular}{lll}
& \textbf{VAE IIA} & \textbf{LDA IIA} \\[0.2em]
\textbf{Correlation with dimensionality:} & $\rho = -0.48$ & $\rho = 0.29$ \\[0.2em]
\textbf{Significance:} & $p = 1.5 \times 10^{-5}$ & $p = 0.014$ \\[0.2em]
\textbf{Sample:} & $n = 73$ regions & $n = 73$ regions \\
\end{tabular}
}
\end{center}
...
The VAE \textbf{reverses and strengthens} the effect: LDA works better in low-dimensional regions (positive $\rho$), but the VAE works better in high-dimensional regions (negative $\rho$). The causal signal was always there---LDA just cannot find it in high-dimensional spaces.

\vspace{0.4em}
{\small \textbf{Why this matters:} dimensionality is not a nuisance variable. It tells you which causal discovery tool to trust.}
\end{frame}
...
\textbf{Measure:} & VAE IIA vs.\ structural connectivity \\
\end{tabular}
\end{center}

\vspace{1em}

\textbf{No.} Structural connectivity and functional evidence routing are independent. Our IIA results are not trivially explained by which regions happen to be well-connected anatomically.
...
2 & Dimensionality predicts causal strength & \textcolor{pass}{\textbf{Confirmed}} & $\rho = -0.48$, $p = 1.5 \times 10^{-5}$ \\[0.5em]
...
\begin{enumerate}
  ...
  \item Rotation everywhere (72/73), but 50$\times$ variation by region type
\end{enumerate}
\end{column}
\begin{column}{0.48\textwidth}
...
%% ============================================================
%% PART 2: NEW RESULTS — deeper validation
%% ============================================================
...
\begin{center}
  ...
  \fcolorbox{black}{white}{\parbox{0.7\textwidth}{\centering\vspace{0.8em}
    ...
    {\large Going deeper: validation and extensions}
  \vspace{0.8em}}}
\end{center}
\vfill
\end{frame}
...
%% --- SLIDE: Shuffled-label control ---
\begin{frame}{But wait: does the VAE learn \emph{real} structure?}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Shuffled-label control}\\[0.3em]
{\small Train the VAE with randomly shuffled choice labels on real neural data. If it learns structure anyway, IIA is meaningless.}
...
\begin{tabular}{lc}
\toprule
\textbf{Condition} & \textbf{Mean IIA} \\
\midrule
Real labels & $\mathbf{0.625}$ \\
...
\vspace{0.3em}
{\small Real $>$ shuffled in \textbf{all 24/24 regions}\\
Wilcoxon $p = 6 \times 10^{-8}$}
\end{column}
\begin{column}{0.50\textwidth}
\textbf{Linear ablation}\\[0.3em]
{\small Is the nonlinearity doing the work, or the structured split?}

\vspace{0.5em}
{\small Nonlinear $-$ linear structured: $\Delta\text{IIA} = +0.001$, $p = 0.65$}

\vspace{0.5em}
\textbf{Verdict:}
\begin{itemize}
  \item The VAE learns \emph{real} structure ($\Delta = 0.195$ over shuffled)
  \item But nonlinearity adds nothing---it's the \textbf{structured split} that matters
  \item This is why linear DAS is safe: same split, no vacuity risk
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{enumerate}\setlength\itemsep{0.4em}
  ...
  \item \textbf{Shuffled-label control} --- real $>$ shuffled in all 24 regions ($p = 6 \times 10^{-8}$)
  \item \textbf{Multi-task generalization} --- four task variables, distinct subspaces
  \item \textbf{Engagement disentanglement} --- orthogonal subspaces, structural test
  \item \textbf{Cross-region patching} --- 1,438 directed pairs, thalamic hubs emerge
...
\vspace{1em}
{\small \textbf{LDA is anti-correlated with causal importance.} Where silencing matters most, linear methods fail.}

\vspace{0.5em}
{\small Independent of IIA vacuity---comparing \emph{between} methods on an external benchmark.}
\end{column}
\end{columns}
\end{frame}

%% --- SLIDE: Engagement disentanglement ---
\begin{frame}{2. Engagement vs.\ choice: the VAE separates what matters}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{The problem:}\\[0.2em]
{\small Miss trials look different from hits. Is that a different \emph{choice computation} or just lower \emph{engagement}?}

\vspace{0.3em}
\textbf{The test:}\\[0.2em]
{\small Train a VAE with separate $\mathbf{z}_{\text{engage}}$ and $\mathbf{z}_{\text{choice}}$ subspaces. If nearly orthogonal, engagement and choice are separated.}

\vspace{0.3em}
\textbf{Result:}\\[0.2em]
{\small Cross-IIA (swap engagement, measure choice flip): only 17\% vs.\ within-subspace $\sim$60\%.}

\vspace{0.2em}
{\small Grassmannian distance $= 2.41$ (near-maximal out of $2.72$), confirming near-orthogonality.}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Why this matters:}\\[0.3em]

\includegraphics[width=0.95\textwidth]{engagement_diagram.pdf}

\vspace{0.3em}
{\small Choice and engagement are orthogonal axes. Linear methods conflate them; the VAE separates them.}
\end{column}
\end{columns}
\end{frame}

%% --- SLIDE: Multi-task generalization ---
\begin{frame}{3. Multi-task generalization: subspaces for four variables}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
...
\vspace{-0.3em}
{\small The VAE's IIA number (0.689) is not itself evidence. But the paper's claims rest on six independent lines:}
\vspace{0.2em}
{\small
\begin{enumerate}\setlength\itemsep{1pt}
  \item \textbf{Shuffled-label control.} Real IIA $>$ shuffled in all 24 regions ($p = 6 \times 10^{-8}$). The VAE learns real structure, not fitting noise.
  \item \textbf{Linear ablation.} Nonlinearity adds nothing ($p = 0.65$). The structured split does the work---which is the safe part.
  \item \textbf{Optogenetic validation is external.} LDA is \emph{anti-correlated} with causal importance ($\rho = -0.82$, $p = 0.023$). Not from IIA.
  \item \textbf{Cross-region patching.} 1,438 directed pairs reveal thalamic causal hubs. External causal test, no IIA involved.
  \item \textbf{VAE circuits.} Choice dimensions carry $2.73\times$ more causal effect---not an artifact of labeling ($2.72\times$ over random split).
  \item \textbf{The comparison is the finding.} Linear methods are anti-correlated with ground truth where it matters most.
\end{enumerate}
}
\end{frame}
...
{\small (Kaufman et al.\ 2014, Gallego et al.\ 2017)}
...
\vspace{0.5em}
{\small Exp67 confirms: null-space fraction predicts linear method failure. High-dimensional regions have large null spaces that dominate CKA.}
\end{column}
\end{columns}
\end{frame}

%% --- SLIDE: Debiased CKA ---
\begin{frame}{Is the CKA result biased?}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Murphy et al.\ (2024):}\\[0.2em]
{\small Standard CKA uses biased HSIC, inflating similarity when trials $\ll$ neurons---exactly our regime.}
...
\begin{tabular}{lcc}
\toprule
\textbf{Estimator} & \textbf{$\rho$} & \textbf{Partial $\rho$} \\
\midrule
Biased CKA & $0.850$ & $0.856$ \\
Debiased CKA & $\mathbf{0.445}$ & $\mathbf{0.342}$ \\
\bottomrule
\end{tabular}
}

\vspace{0.3em}
{\small $\Delta\rho = -0.40$: biased CKA overstates the dissociation.}
\end{column}
\begin{column}{0.50\textwidth}
\textbf{But the dissociation is real:}\\[0.3em]
{\small
\begin{itemize}\setlength\itemsep{3pt}
  \item Debiased $\rho = 0.445$ still highly significant ($p < 10^{-60}$)
  \item The anti-correlation with Grassmannian distance \emph{strengthens} after debiasing
  \item Biased CKA inflated the \emph{agreement} between CKA and Procrustes---removing inflation makes them disagree \emph{more}
\end{itemize}
}

\vspace{0.3em}
{\small \textbf{Takeaway:} The metric dissociation holds. Standard CKA overstated it, but the pattern is robust.}
\end{column}
\end{columns}
\end{frame}

%% --- SLIDE: Alpha bias ---
\begin{frame}{The dimensionality confound}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Chun et al.\ (2025):}\\[0.2em]
{\small Power-law $\alpha$ is confounded with neuron count (finite-sample bias in eigenvalue estimation).}
...
\begin{tabular}{lcc}
\toprule
\textbf{Covariate} & $\rho$ \textbf{with} $\alpha$ & $p$ \\
\midrule
$n_{\text{neurons}}$ & $-0.908$ & $10^{-103}$ \\
...
\vspace{0.3em}
{\small Massive confound ($|\rho| = 0.91$): regions with more neurons show lower $\alpha$.}
\end{column}
\begin{column}{0.50\textwidth}
\textbf{Impact on our claims:}\\[0.3em]
{\small
\begin{itemize}\setlength\itemsep{3pt}
  \item $\alpha$ alone cannot be trusted as a dimensionality measure
  \item Must report partial correlations controlling for $n_{\text{neurons}}$
  \item Trial count is not confounded ($p = 0.08$)
  \item Our main finding (CKA--Procrustes dissociation) does \emph{not} depend on $\alpha$---it uses CKA and Procrustes directly
\end{itemize}
}

\vspace{0.3em}
{\small \textbf{Lesson:} Always control for recording yield when using spectral dimensionality measures.}
\end{column}
\end{columns}
\end{frame}

%% --- SLIDE: Cross-region patching ---
\begin{frame}{Biological path patching: 1,438 directed pairs}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Method:}\\[0.2em]
{\small For region pair $A \to B$: replace $B$'s choice-subspace projection with $A$'s, measure if decoder flips.}

\vspace{0.3em}
\textbf{Top hub regions (by outgoing IIA):}\\[0.2em]
{\scriptsize
\begin{tabular}{lccc}
\toprule
\textbf{Region} & \textbf{Out} & \textbf{In} & \textbf{Asym.} \\
\midrule
...
\end{column}
\begin{column}{0.50\textwidth}
\textbf{What this reveals:}\\[0.3em]
{\small
\begin{itemize}\setlength\itemsep{3pt}
  \item Thalamic regions (MG, MD) are \textbf{causal sources}---positive asymmetry means their choice signal propagates \emph{out}
  \item Visual/striatal regions are \textbf{net receivers}---negative asymmetry
  \item Hippocampal pairs (CA3$\to$LS, SUB$\to$LS) achieve \textbf{IIA $= 1.0$}---perfect choice transfer
  \item Directed graph from geometry alone, no causal model needed
\end{itemize}
}

\vspace{0.3em}
{\small This is the first cross-region causal geometry test in neural data.}
\end{column}
\end{columns}
\end{frame}

%% --- SLIDE: VAE causal circuits ---
\begin{frame}{Does the VAE develop specialized ``choice detectors''?}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{VAE encoder circuit analysis}\\[0.2em]
{\small (following Martinez et al.\ 2025)}
...
Choice/other ratio & $\mathbf{2.73\times}$ \\
Random-split ratio & $1.00 \pm 0.02$ \\
Lift over random & $\mathbf{2.72\times}$ \\
\bottomrule
\end{tabular}
}
\end{column}
\begin{column}{0.50\textwidth}
\textbf{Interpretation:}\\[0.3em]
{\small
\begin{itemize}\setlength\itemsep{3pt}
  \item Choice dimensions carry $2.73\times$ more causal effect than ``other'' dimensions
  \item This is $2.72\times$ more than a \textbf{random split} of the same dimensions would give
  \item The structured split doesn't just label dimensions---it concentrates causal signal
  \item The VAE develops genuine specialization, not an artifact of labeling
\end{itemize}
}

\vspace{0.3em}
{\small Modularity: structured $0.86$, unstructured $0.93$---the forced split slightly reduces modularity but concentrates choice information.}
\end{column}
\end{columns}
\end{frame}

%% --- SLIDE: pi-VAE result ---
\begin{frame}{Does adding identifiability help?}
\textbf{pi-VAE hybrid ablation} (73 regions):
...
\begin{tabular}{lcc}
\toprule
\textbf{Model} & \textbf{Mean IIA} & \textbf{vs.\ Structured VAE} \\
\midrule
\textbf{Structured VAE} & $\mathbf{0.941}$ & --- \\
...
\vspace{0.8em}
\begin{flushleft}
\textbf{Surprise:} Adding identifiability constraints (label-conditioned prior) \emph{destroys} performance. The structured split alone is sufficient; the prior is unnecessary and actively harmful.

\vspace{0.3em}
\textbf{Why?} The binary choice label ($k = 2$) provably fails pi-VAE's identifiability condition: two classes produce at most rank~1 in the prior mean space, but $d_{\text{causal}} = 3$ requires rank~3. The prior over-regularizes because it cannot represent the causal subspace.
\end{flushleft}
\end{frame}

%% --- SLIDE: Updated validity scorecard ---
\begin{frame}{Updated validity scorecard}
\vspace{-0.5em}
{\scriptsize
\begin{tabular}{lp{7cm}c}
\toprule
...
\textbf{Specificity} & Only evidence axis flips; RT, feedback, random do not & \textcolor{pass}{\textbf{Pass}} \\
\textbf{Method invariance} & 4 intervention types agree (avg.\ correlation $0.72$) & \textcolor{pass}{\textbf{Pass}} \\
\textbf{Confound control} & No confound above $0.5$; neuron count $\rho = -0.48$ & \textcolor{pass}{\textbf{Pass}} \\
\textbf{Graded response} & IIA monotonic in 80\% of regions & \textcolor{pass}{\textbf{Pass}} \\
\textbf{Double dissociation} & Evidence $\perp$ RT subspaces & \textcolor{pass}{\textbf{Pass}} \\
\textbf{Learned subspaces} & VAE IIA 3.4$\times$ LDA; wins 73/73 regions & \textcolor{pass}{\textbf{Pass}} \\
\textbf{Engagement separation} & $\mathbf{z}_{\text{engage}} \perp \mathbf{z}_{\text{choice}}$; cross-IIA only 17\% & \textcolor{pass}{\textbf{Pass}} \\
\textbf{Multi-task} & 4 task variables with distinct, structured subspaces & \textcolor{pass}{\textbf{Pass}} \\
\textbf{Optogenetic} & LDA anti-corr.\ with silencing: $\rho = -0.82$, $p = 0.023$ & \textcolor{pass}{\textbf{Pass}} \\
\textbf{Shuffled-label control} & Real IIA $>$ shuffled in 24/24 regions ($p = 6 \times 10^{-8}$) & \textcolor{pass}{\textbf{Pass (new)}} \\
\textbf{Linear ablation} & Nonlinearity adds nothing ($\Delta = 0.001$, $p = 0.65$) & \textcolor{pass}{\textbf{Pass (new)}} \\
\textbf{Cross-region patching} & 1,438 pairs; thalamic hubs emerge; hippocampal IIA $= 1.0$ & \textcolor{pass}{\textbf{Pass (new)}} \\
\textbf{VAE circuits} & Choice dims carry $2.73\times$ CES vs.\ other ($2.72\times$ over random) & \textcolor{pass}{\textbf{Pass (new)}} \\
\textbf{Debiased CKA} & Dissociation holds ($\rho = 0.45$, $p < 10^{-60}$) after bias correction & \textcolor{pass}{\textbf{Pass (new)}} \\
\textbf{IIA vacuity check} & VAE on random noise $= 0.699$ (vacuous!); external validation needed & \textcolor{fail}{\textbf{Fail}} \\
\textbf{pi-VAE hybrid} & Prior destroys structured VAE (IIA $0.036$ vs.\ $0.941$) & \textcolor{partial}{\textbf{Informative}} \\
\bottomrule
\end{tabular}
}

\vspace{0.3em}
{\small 16 pass, 1 fail (IIA vacuity), 1 informative negative (pi-VAE).}
\end{frame}
...
\begin{itemize}
  \item \textbf{IIA is vacuous} for nonlinear methods---shuffled-label control resolves this ($p = 6 \times 10^{-8}$), but the underlying metric remains suspect
  \item \textbf{CKA inflated} by biased HSIC---debiasing cuts $\rho$ from $0.85$ to $0.45$ (dissociation holds but is weaker)
  \item \textbf{$\alpha$ confounded} with neuron count ($\rho = -0.91$)---all $\alpha$ claims need partial correlations
  \item \textbf{Small silencing overlap}: VAE needs $n > 60$ for 80\% power (have $n = 12$)
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{What we did about it:}
\begin{itemize}
  \item Shuffled-label control confirms real structure
  \item Linear ablation shows split, not nonlinearity, does the work
  \item Cross-region patching provides external causal test (1,438 pairs, thalamic hubs)
  \item VAE circuits show $2.73\times$ CES concentration in choice dims
  \item pi-VAE hybrid: identifiability prior not helpful (IIA $0.036$)
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{columns}[T]
\begin{column}{0.5\textwidth}
{\small \textbf{Established findings:}}
{\small
\begin{enumerate}\setlength\itemsep{1pt}
  \item CKA--Procrustes anti-correlation holds after debiasing ($\rho = 0.45$, $p < 10^{-60}$)
  \item Dimensionality mediates the dissociation (partial $r = +0.44$)
  \item Evidence subspace causally mediates choice (4 methods agree, $r = 0.72$)
  \item Structured split, not nonlinearity, does the work ($\Delta = 0.001$, $p = 0.65$)
  \item Shuffled-label control confirms genuine structure ($p = 6 \times 10^{-8}$)
  \item Cross-region patching reveals thalamic causal hubs
\end{enumerate}
}
\end{column}
\begin{column}{0.48\textwidth}
{\small \textbf{Critical nuances:}}
{\small
\begin{enumerate}\setlength\itemsep{1pt}
  \setcounter{enumi}{6}
  \item \textcolor{fail}{IIA is vacuous for nonlinear methods}---but shuffled control and linear ablation resolve this
  \item LDA is \emph{anti-correlated} with optogenetic importance ($\rho = -0.82$, $p = 0.023$)
  \item $\alpha$ confounded with $n_{\text{neurons}}$ ($\rho = -0.91$)---must control for recording yield
  \item VAE encoder develops specialized choice channels ($2.73\times$ CES)
\end{enumerate}
}

\vspace{0.3em}
{\small \textbf{Takeaway:} Linear methods are anti-correlated with ground truth. The structured split, not nonlinearity, is what matters.}
\end{column}
\end{columns}
\end{frame}
...
\begin{enumerate}\setlength\itemsep{0.6em}
  \item \textbf{Neural geometry is not metric-neutral.} CKA and Procrustes rank the same brain regions in opposite order---which tool you pick determines what you find.
  \item \textbf{Linear methods are anti-correlated with causal importance.} LDA-based subspaces anti-correlate with optogenetic ground truth ($\rho = -0.82$).
  \item \textbf{Dimensionality tells you which tool to trust.} High-dimensional regions have large null spaces that dominate CKA but not Procrustes.
  \item \textbf{A structured split---not nonlinearity---is what matters.} The VAE's improvement comes from separating choice from nuisance dimensions, not from being nonlinear ($p = 0.65$).
\end{enumerate}

\vspace{0.5em}
{\small Check your metrics against external ground truth. If you don't, you won't know which tool is lying.}
\end{frame}
...
\begin{center}
\vfill
{\Large Causal Geometric Structure in Neural Populations}
...
{\scriptsize
\begin{itemize}\setlength\itemsep{2pt}
  \item Steinmetz, N.\ A.\ et al.\ (2019). Distributed coding of choice, action and engagement across the mouse brain. \emph{Nature}, 576, 266--273.
  \item Kornblith, S.\ et al.\ (2019). Similarity of neural network representations revisited. \emph{ICML}.
  \item Sch\"onemann, P.\ H.\ (1966). A generalized solution of the orthogonal Procrustes problem. \emph{Psychometrika}, 31(1), 1--10.
  \item Geiger, A.\ et al.\ (2021). Causal abstractions of neural networks. \emph{NeurIPS}.
  \item Kaufman, M.\ T.\ et al.\ (2014). Cortical activity in the null space. \emph{Nature Neuroscience}, 17, 440--448.
  \item Gallego, J.\ A.\ et al.\ (2017). Neural manifolds for the control of movement. \emph{Neuron}, 94(5), 978--984.
  \item Zhou, D.\ \& Wei, X.-X.\ (2020). Learning identifiable and interpretable latent models of high-dimensional neural activity using pi-VAE. \emph{NeurIPS}.
  \item Sutter, T.\ et al.\ (2025). On the Vacuity of IIA for Nonlinear Causal Abstraction. \emph{NeurIPS} (Spotlight).
  \item Huang, B.\ et al.\ (2020). Causal discovery from heterogeneous/nonstationary data. \emph{JMLR}, 21(89), 1--53.
\end{itemize}
}
\end{frame}
