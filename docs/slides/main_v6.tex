...
\vspace{0.5em}
\textcolor{choice}{\textbf{Status:}} In progress.
\end{column}
\end{columns}
\end{frame}
...
\vspace{0.3em}
\textbf{Result:}\\[0.2em]
{\small Cross-IIA (swap engagement, measure choice flip): only $\sim$13\% vs.\ within-subspace $\sim$60\%.}

\vspace{0.2em}
{\small Grassmannian distance $\approx 2.35$ (near-maximal), confirming near-orthogonality.}
\end{column}
\begin{column}{0.48\textwidth}
...
\vspace{0.5em}
{\small \textcolor{choice}{\textbf{Running:}} Exp67 decomposes each region into potent/null and tests whether null-space fraction predicts linear method failure.}
\end{column}
\end{columns}
\end{frame}
...
\vspace{0.3em}
\textbf{Why?} The binary choice label ($k = 2$) barely meets pi-VAE's identifiability condition. With so little supervision signal, the prior over-regularizes the latent space.
\end{flushleft}
\end{frame}
...
\textbf{Engagement separation} & $\mathbf{z}_{\text{engage}} \perp \mathbf{z}_{\text{choice}}$; cross-IIA only 13\% & \textcolor{pass}{\textbf{Pass}} \\
...
\begin{itemize}\setlength\itemsep{2pt}
  ...
  \item Deng, S.\ \& Gu, Y.\ (2023). On the optimal rate for the convergence problem in mean field control. \emph{JMLR}.
