...
\subtitle{Resolving the linear-nonlinear divide in neural analysis tools}
%\subtitle{...\\[0.3em]{\small Exploratory results on Neuropixels visual decision-making data}}
\author{Elliot Tower}
\date{2026}
\institute{}
...
\begin{enumerate}
  ...
  \item Learned subspaces are more accurate
\end{enumerate}
}
\end{column}
\begin{column}{0.22\textwidth}
\textbf{Part 2: Stress-testing}
{\small
\begin{enumerate}
  \setcounter{enumi}{4}
  \item Validity checks
  \item IIA vacuity problem
  \item External validation
  \item Engagement separation
\end{enumerate}
}
\end{column}
\begin{column}{0.22\textwidth}
\textbf{Part 3: Deeper analysis}
{\small
\begin{enumerate}
  \setcounter{enumi}{8}
  \item Neural manifold framework
  \item Activation patching
  \item Choice information flow
\end{enumerate}
}
\end{column}
\begin{column}{0.26\textwidth}
\textbf{Part 4: Additional testing}
{\small
\begin{enumerate}
  \setcounter{enumi}{11}
  \item Per-variable causal subspaces
  \item Optogenetic: linear vs nonlinear
  \item Grassmannian dissimilarity
  \item Encoder specialization
\end{enumerate}
}
\end{column}
\end{columns}
...
\begin{itemize}\setlength\itemsep{3pt}
  \item {\small \textbf{Structured representation learning} (\href{https://proceedings.mlr.press/v89/esmaeili19a}{Esmaeili et al.\ 2019}; Opper et al.\ 2023; ICML 2025)---structural priors improve identifiability. Our metric taxonomy is an inductive bias argument.}
...
\end{column}
\begin{column}{0.48\textwidth}
\textbf{3. Causal interventions (IIA)}\\[0.2em]
...
\vspace{0.6em}
\textbf{4. Learned subspaces are more accurate}\\[0.2em]
...
%% ============================================================
\section{3. Causal interventions (IIA)}
...
\vspace{0.3em}
{\small \textbf{The real test:} cross-dataset replication (IBL data, different mice, different labs). If the same subspaces work there, it's not an artifact of our pipeline. Planned.}
\end{column}
\end{columns}
\end{frame}
...
\section{4. Learned subspaces are more accurate}
...
\vspace{0.5em}
{\small \textbf{Takeaway:} Neural geometry is not metric-neutral---the tool you choose determines what you find, and dimensionality tells you which tool to trust. The causal signal is 3.4$\times$ stronger in learned subspaces than linear projections reveal.}
\end{frame}
...
\section{6. The IIA vacuity problem}
...
\vspace{0.5em}
{\small (Engagement disentanglement is covered separately in \S8.)}
\end{frame}
...
\begin{center}
  ...
  \fcolorbox{black}{white}{\parbox{0.7\textwidth}{\centering\vspace{0.8em}
    ...
    {\large Deeper analysis: mechanisms and circuits}
  \vspace{0.8em}}}
\end{center}
\vfill
\end{frame}
...
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{9. The neural manifold framework}\\[0.2em]
...
\vspace{0.6em}
\textbf{10. Activation patching}\\[0.2em]
...
\end{column}
\begin{column}{0.48\textwidth}
\textbf{11. Choice information flow}\\[0.2em]
...
\section{9. The neural manifold framework}
...
\vspace{0.3em}
{\small Both decode equally from potent space. But LDA sees almost nothing \emph{except} null variance.}
\end{column}
...
\vspace{0.5em}
{\small $\alpha$ anti-correlates with null-space fraction:}
...
\section{10. Activation patching}
...
\vspace{0.3em}
{\small \textbf{Scale:} 1,438 directed pairs, 39 sessions, 73 regions.}
...
\section{11. Choice information flow}
...
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{12. Per-variable causal subspaces}\\[0.2em]
...
\vspace{0.6em}
\textbf{13. Optogenetic: linear vs nonlinear}\\[0.2em]
{\small Full method comparison against silencing ground truth. LDA is actively misleading ($\rho = -0.82$); VAE and decoding are not.}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{14. Grassmannian dissimilarity}\\[0.2em]
{\small A neural taskonomy from subspace geometry. LDA and VAE dissimilarity matrices correlate at $\rho = 0.97$ and anti-correlate with CKA.}

\vspace{0.6em}
\textbf{15. Encoder specialization}\\[0.2em]
{\small The structured VAE develops dedicated choice-detector channels. Specialization emerges from the bottleneck.}
\end{column}
\end{columns}
\end{frame}

\section{12. Per-variable causal subspaces}
...
\section{13. Optogenetic ground truth: linear vs nonlinear}

%% --- SLIDE: Optogenetic ---
\begin{frame}{LDA is anti-correlated with causal importance}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\textbf{The external test:} Zatka-Haas et al.\ (2021) silenced cortical regions with optogenetics and measured behavioral impact. This is \textbf{ground-truth causal importance} --- completely independent of our geometry pipeline.

\vspace{0.5em}
\textbf{Result:} LDA IIA is \emph{negatively} correlated with silencing effect ($\rho = -0.82$, $p = 0.023$, $n = 7$ grouped regions). Regions where LDA says choice information is strongest are the ones where silencing has the \emph{least} effect.
\end{column}
...
\begin{tabular}{lcc}
\toprule
\textbf{Method} & \textbf{$\rho$} & \textbf{$p$} \\
\midrule
LDA IIA & $\mathbf{-0.82}$ & $\mathbf{0.023}$ \\
VAE IIA & $-0.50$ & 0.25 \\
...
\vspace{0.5em}
{\small \textbf{Key:} This is external validation --- silencing effects come from a different experiment, different mice, different lab. LDA doesn't just underperform; it is \emph{actively misleading}. Small $n$ ($= 7$ grouped) limits power; bootstrap CI is wide. More silencing data would sharpen this.}
\end{column}
\end{columns}
\end{frame}

\section{14. Grassmannian dissimilarity}

%% --- SLIDE: Grassmannian dissimilarity matrices ---
\begin{frame}{Grassmannian dissimilarity: a neural taskonomy}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\textbf{Method:} Compute $73 \times 73$ pairwise Grassmannian distances between subspaces found by LDA, structured VAE, and CKA.

\vspace{0.5em}
\textbf{Result:} LDA and VAE dissimilarity matrices are nearly identical ($\rho = 0.97$). Both \emph{anti-correlate} with CKA.
...
\section{15. Encoder specialization}
...
\vspace{0.5em}
{\small The structural bottleneck doesn't just improve IIA scores---it forces the encoder to develop genuinely specialized channels. The architecture creates interpretability.}
\end{column}
\end{columns}
\end{frame}
...
\textbf{Potent/null decomp.} & Null-space fraction anti-correlates with dissociation ($\rho = -0.86$) & \textcolor{pass}{\textbf{Pass (new)}} \\
\textbf{Identifiable subspaces} & pi-VAE identifiability prior kills causal signal entirely (IIA 0.026) & \textcolor{fail}{\textbf{Fail (new)}} \\
\textbf{Cross-region patching} & 1,438 directed pairs; sender/receiver hub structure in 73 regions & \textcolor{pass}{\textbf{Pass (new)}} \\
\bottomrule
\end{tabular}
}

\vspace{0.3em}
{\small 13 pass, 2 fail (IIA vacuity + pi-VAE identifiability kills signal). Honest negatives strengthen the case.}
\end{frame}
...
\begin{itemize}
  \item \textbf{One dataset} (Steinmetz 2019, 10 mice)---needs IBL replication
  \item \textbf{Binary task only}---4-variable generalization helps but not sufficient
  \item \textbf{Small silencing overlap}: $n = 12$ matched regions---bootstrap CI is wide, cannot shrink without more data
  \item \textbf{10 mice}: individual differences unquantifiable at this $n$
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Addressable with more compute:}
\begin{itemize}
  \item \textbf{IIA vacuity}: bootstrapping over sessions can tighten CI on real vs random gap
  \item \textbf{Cross-region patching}: bootstrap over session subsets for edge stability
  \item \textbf{VAE training variance}: ensembles of 5--10 seeds per region (currently 1)
  \item \textbf{IBL replication}: same pipeline, different dataset---planned
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{enumerate}
  ...
  \item The real finding is the \emph{comparison}: linear methods are actively wrong in high-dimensional regions
\end{enumerate}

\vspace{0.3em}
{\small \textbf{Takeaway:} Dimensionality predicts where linear methods fail.}
\end{column}
\end{columns}
\end{frame}
...
{\small Linear methods are anti-correlated with causal importance.}

\vspace{0.2em}

{\small Learned subspaces recover 3.4$\times$ more causal signal ---}

\vspace{0.1em}

{\small but only external ground truth can tell you if you're right.}
\end{center}
\end{frame}
...
\textbf{Experiments:} 72 experiments on Modal A10G GPUs; all results in \texttt{results/}
}
\end{frame}
...
\begin{itemize}\setlength\itemsep{0pt}
  ...
  \item Geiger et al.\ (2025). \href{https://arxiv.org/abs/2301.04709}{Causal abstraction: A theoretical foundation for mechanistic interpretability.} \textit{JMLR}.
