...
\textbf{Structured VAE architecture.} We train a structured variational autoencoder \citep{narayanaswamy2017learning} per region. The architecture enforces an explicit factorization of the latent space into a 3-dimensional ``choice'' subspace and a 15-dimensional ``other'' subspace. Only the choice latent is trained to predict the animal's left/right decision (via an auxiliary classification loss with weight $\alpha = 10$), while both latent groups jointly reconstruct the population activity. This design encourages the choice subspace to capture the minimal sufficient representation for the decision while the other subspace absorbs remaining variance. The model trains for 300 epochs per region across all available sessions ($n = 39$), achieving near-perfect choice classification from the 3-dimensional choice latent alone (mean accuracy $= 0.99$ across 73 regions).
