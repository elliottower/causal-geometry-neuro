...
\textbf{Part 1: Core results}
\begin{enumerate}
  \item Does population shape matter?
  \item Linear vs.\ nonlinear disagree
  \item Causal interventions (IIA)
  \item Learned subspaces are stronger
  \item Validity checks and predictions
\end{enumerate}
\end{column}
\begin{column}{0.32\textwidth}
\textbf{Part 2: Validation}
\begin{enumerate}
  \setcounter{enumi}{5}
  \item Neural manifold framework
  \item Potent/null decomposition
  \item The IIA vacuity problem
  \item External validation lines
\end{enumerate}
\end{column}
\begin{column}{0.32\textwidth}
\textbf{Part 3: Circuit analysis}
\begin{enumerate}
  \setcounter{enumi}{9}
  \item Activation patching (MI $\to$ neuro)
  \item Choice information flow network
  \item Sender/receiver hub structure
\end{enumerate}
\end{column}
\end{columns}
\end{frame}
...
%% --- SLIDE: The data ---
\begin{frame}{The data: \href{https://doi.org/10.1038/s41586-019-1787-x}{Steinmetz et al.\ 2019}}
\begin{columns}[T]
...
\vspace{0.3em}
{\small Validity framework from philosophy of science (Mayo 2018, Craver 2007, Shallice 1988) and causal inference (\href{https://arxiv.org/abs/1203.3519}{Pearl \& Bareinboim 2011}, \href{https://doi.org/10.1093/0195155270.001.0001}{Woodward 2003}).}
\end{frame}
...
\vspace{0.6em}
{\small Criteria adapted from \href{https://doi.org/10.1093/acprof:oso/9780199299317.001.0001}{Craver (2007)}, \href{https://doi.org/10.1017/9781107286184}{Mayo (2018)}, and \href{https://doi.org/10.1017/CBO9780511526817}{Shallice (1988)}. Pass/fail thresholds were pre-registered to keep each criterion falsifiable and prevent post-hoc relabeling.}
\end{frame}
...
\section{The neural manifold framework}
...
\vspace{0.5em}
{\small \textbf{Exp67 confirms this:} Null-space fraction strongly anti-correlates with metric dissociation ($\rho = -0.86$, $p = 5 \times 10^{-22}$).}
\end{column}
\end{columns}
\end{frame}
...
\textbf{The idea} {\scriptsize (\href{https://doi.org/10.1038/nn.3643}{Kaufman et al.\ 2014})}
...
\begin{itemize}\setlength\itemsep{0.3em}
  ...
  \item {\small \textbf{\textcolor{neutral}{Null space}}: high-variance activity orthogonal to choice --- present but not causal}
\end{itemize}
...
\vspace{0.5em}
{\small Both decode choice equally well from the potent subspace. But LDA sees almost nothing \emph{except} null variance.}
\end{column}
...
\begin{tikzpicture}[scale=0.85]
  ...
  \fill[neutral!8] (0,0) ellipse (2.5 and 2.5);
  \draw[neutral,thick] (0,0) ellipse (2.5 and 2.5);
  \node[black,font=\scriptsize\bfseries] at (0,2.0) {null space};
  \node[black,font=\tiny] at (0,1.6) {(99.8\% of LDA variance)};

  % Potent space (thin ellipse)
  ...
  % Annotations
  \draw[neutral,thick,-{Stealth[length=5pt]}] (-3.2,2.2) -- (-1.8,1.5);
  \node[neutral,font=\tiny\bfseries,anchor=south east,text width=1.8cm,align=right] at (-3.0,2.2) {CKA dominated by this};

  \draw[choice,thick,-{Stealth[length=5pt]}] (3.2,-1.5) -- (1.6,-0.3);
  \node[choice,font=\tiny\bfseries,anchor=north west,text width=2cm] at (3.0,-1.5) {VAE finds this};
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
...
\textbf{The geometric dissociation index $\alpha$} measures how much linear and nonlinear tools disagree in each region.

\vspace{0.5em}
{\small $\alpha$ is strongly anti-correlated with null-space fraction:}

\vspace{0.3em}
{\small
\begin{tabular}{lcc}
\toprule
\textbf{Method} & $\boldsymbol{\rho}$ & $\boldsymbol{p}$ \\
\midrule
VAE & $-0.86$ & $5 \times 10^{-22}$ \\
LDA & $-0.68$ & $3 \times 10^{-11}$ \\
\bottomrule
\end{tabular}
}

\vspace{0.5em}
{\small Regions where tools disagree most $=$ regions with the \textbf{largest potent subspaces}.}

\vspace{0.5em}
{\small But potent decoding accuracy does \emph{not} correlate with $\alpha$ (VAE: $\rho = -0.07$, $p = 0.57$). The signal is always there --- only its \textbf{relative size} varies.}
\end{column}
...
\vspace{0.5em}
\begin{tikzpicture}[scale=0.7]
  % Low alpha region (small potent, big null)
  \node[font=\scriptsize\bfseries] at (0,3.8) {Low $\alpha$ (tools agree)};
  \fill[neutral!8] (0,2) ellipse (1.5 and 1.5);
  \draw[neutral,thick] (0,2) ellipse (1.5 and 1.5);
  \fill[choice!25] (0,2) ellipse (0.3 and 0.1);
  \draw[choice,thick] (0,2) ellipse (0.3 and 0.1);
  \node[font=\tiny,neutral] at (0,0.2) {huge null, tiny potent};

  % High alpha region (big potent, less null)
  \node[font=\scriptsize\bfseries] at (5,3.8) {High $\alpha$ (tools disagree)};
  \fill[neutral!8] (5,2) ellipse (1.5 and 1.5);
  \draw[neutral,thick] (5,2) ellipse (1.5 and 1.5);
  \fill[choice!25] (5,2) ellipse (1.1 and 0.4);
  \draw[choice,ultra thick] (5,2) ellipse (1.1 and 0.4);
  \node[font=\tiny,neutral] at (5,0.2) {less null, large potent};

  % Arrow
  \draw[black,ultra thick,-{Stealth[length=6pt]}] (1.8,2) -- (3.2,2);
  \node[font=\tiny\bfseries] at (2.5,2.4) {$\alpha$ increases};
\end{tikzpicture}
...
\begin{frame}{Roadmap}
\vspace{0.3em}
The VAE's IIA number (0.689) is not itself evidence. But the paper's claims don't rest on that number alone. Four independent lines of evidence:
...
\begin{enumerate}
  ...
  \item \textbf{Engagement disentanglement.} The VAE separates engagement from choice into orthogonal subspaces (cross-IIA only 13\%). Structural, not metric-based.
...
\begin{center}
  ...
  \fcolorbox{black}{white}{\parbox{0.7\textwidth}{\centering\vspace{0.8em}
    ...
    {\large Circuit analysis: mapping choice information flow}
  \vspace{0.8em}}}
\end{center}
\vfill
\end{frame}
...
\vspace{0.3em}
{\small Activation patching (also called \emph{causal tracing}) is a core tool in mechanistic interpretability. The idea:}
...
\begin{tikzpicture}[scale=0.75,
  ...
  \node[box,fill=neutral!15] (a2) at (0,2.5) {Layer $\ell$};
  ...
  \node[box,fill=choice!15] (b2) at (4,2.5) {Layer $\ell$};
...
\vspace{0.3em}
{\small Instead of patching between \emph{layers} of one network, we patch between \emph{brain regions} of one mouse:}

\vspace{0.3em}
{\small
\begin{enumerate}\setlength\itemsep{0.3em}
  \item Train a VAE per region $\to$ get $\mathbf{z}_{\text{choice}}$ (choice latent)
  \item Take region A's $\mathbf{z}_{\text{choice}}$ from a left-choice trial
  \item Patch it into region B's decoder for a right-choice trial
  \item Decode choice from region B's patched representation
  \item If the prediction flips $\to$ A and B share \textbf{compatible choice codes}
\end{enumerate}
}

\vspace{0.3em}
{\small This is \textbf{directed}: A$\to$B may work but B$\to$A may not $\to$ reveals sender/receiver roles.}
\end{column}
...
\begin{itemize}\setlength\itemsep{0.5em}
  ...
  \item \textbf{Directionality}: some regions are ``senders'' (their code works elsewhere) and others are ``receivers'' (they accept external codes)
  \item A \textbf{circuit map} of choice information flow --- not from anatomy, but from representational compatibility
\end{itemize}
}

\vspace{0.5em}
{\small \textbf{Scale:} 1,438 directed pairs across 39 sessions, 73 regions.}

\vspace{0.3em}
{\small \textbf{Key difference from ML:} we patch between \emph{separately trained} encoders, so compatibility reflects shared geometry, not shared weights.}
\end{column}
\end{columns}
\end{frame}

\section{Choice information flow}
...
\vspace{0.5em}
{\small \textbf{Key hubs:} ILA (infralimbic), LS (lateral septum), MD (mediodorsal thalamus) are strong senders. CA (hippocampus), ACB (nucleus accumbens) are strong receivers.}
\end{column}
\end{columns}
\end{frame}
...
\vspace{0.5em}
{\small \textbf{Structure visible:} clusters of high transfer (warm blocks) reveal groups of regions sharing compatible choice representations. The matrix is \emph{not symmetric} --- transfer is directional.}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}\setlength\itemsep{0.3em}
  ...
  \item Diagonal = symmetric regions
  \item Above diagonal = net receiver
  \item Below diagonal = net sender
  \item Dot size $\propto$ number of connections
\end{itemize}
}

\vspace{0.5em}
{\small \textbf{Notable asymmetries:}
\begin{itemize}\setlength\itemsep{0.1em}
  \item {\scriptsize AUD: strong sender, weak receiver ($+0.33$)}
  \item {\scriptsize EPd: strong receiver, weak sender ($-0.35$)}
  \item {\scriptsize ILA: strong sender ($0.67$ out)}
\end{itemize}
}

\vspace{0.3em}
{\small Asymmetry may reflect the feedforward flow of choice information from sensory $\to$ decision regions.}
\end{column}
\end{columns}
\end{frame}

%% --- SLIDE: Running and planned experiments ---
\begin{frame}{What's running now}
\vspace{-0.3em}
{\scriptsize
\begin{tabular}{lp{8.5cm}c}
\toprule
\textbf{Exp} & \textbf{What it tests} & \textbf{Status} \\
\midrule
67 & Potent/null space decomposition: does null-space fraction predict where linear methods fail? & \textcolor{causal}{Running} \\
68 & $73 \times 73$ Grassmannian dissimilarity matrix: does subspace similarity recover functional hierarchy? & \textcolor{causal}{Running} \\
69 & pi-VAE hybrid ablation: does adding identifiability guarantees (label-conditioned prior) improve over structured VAE alone? & \textcolor{causal}{Running} \\
70 & Cross-region activation patching: biological path patching---does swapping one region's choice subspace into another flip its decoded choice? & \textcolor{causal}{Running} \\
71 & VAE mechanistic interpretability: CES, specificity, and circuit modularity---does the encoder develop specialized ``choice detector'' channels? & \textcolor{causal}{Running} \\
\bottomrule
\end{tabular}
}
...
\begin{description}
  \item[67] Mechanistic explanation for CKA/Procrustes dissociation
  \item[68] ``Neural taskonomy''---functional hierarchy from geometry
  \item[69] Addresses IIA vacuity via identifiability theory
\end{description}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{New directions:}
\begin{description}
  \item[70] First cross-region causal geometry test
  \item[71] Mech interp of the analysis tool itself---meta-science
\end{description}

\vspace{0.5em}
{\small Results expected within $\sim$1 hour. Slides will be updated with numbers.}
\end{column}
\end{columns}
\end{frame}
...
\textbf{Engagement separation} & $\mathbf{z}_{\text{engage}} \perp \mathbf{z}_{\text{choice}}$; cross-IIA only 13\% & \textcolor{pass}{\textbf{Pass (new)}} \\
\textbf{Multi-task} & 4 task variables with distinct, structured subspaces & \textcolor{pass}{\textbf{Pass (new)}} \\
\textbf{Optogenetic (expanded)} & LDA anti-corr.\ with silencing: $\rho = -0.82$, $p = 0.023$ & \textcolor{pass}{\textbf{Pass (new)}} \\
\textbf{IIA vacuity check} & VAE IIA on random noise $= 0.699$ (vacuous!); external validation needed & \textcolor{fail}{\textbf{Fail (new)}} \\
\textbf{Potent/null decomp.} & Does null-space fraction explain linear method failure? & \textcolor{causal}{\textbf{Running}} \\
\textbf{Identifiable subspaces} & pi-VAE hybrid with identifiability guarantees & \textcolor{causal}{\textbf{Running}} \\
\textbf{Cross-region patching} & Biological path patching between simultaneously recorded regions & \textcolor{causal}{\textbf{Running}} \\
\bottomrule
\end{tabular}
}

\vspace{0.3em}
{\small 11 pass, 1 fail (IIA vacuity---motivates external validation focus), 3 running.}
\end{frame}
...
\begin{itemize}
  \item \textbf{IIA is vacuous} for nonlinear methods---our VAE IIA of 0.689 is not itself evidence of causal structure
  \item \textbf{One dataset} (Steinmetz 2019)---needs replication
  \item \textbf{Binary task only}---though 4-variable generalization helps
  \item \textbf{Small silencing overlap}: $n = 16$ expanded, $n = 7$ grouped
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{What we're doing about it:}
\begin{itemize}
  \item pi-VAE hybrid for identifiable subspaces (exp69, running)
  \item IBL cross-dataset replication
  \item Cross-region activation patching (exp70, running)---external causal test that doesn't use IIA
  \item VAE encoder interpretability (exp71, running)---do specialized channels emerge?
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{enumerate}
  \item Linear and manifold tools anti-correlate ($\rho = -0.85$); dimensionality explains why
  \item Variance structure is universal ($0.94$); neural content is not ($0.09$)
  \item Evidence subspace causally mediates choice (4 intervention methods agree, $r = 0.72$)
...
\begin{enumerate}
  ...
  \item The real finding is the \emph{comparison}: linear methods are actively wrong where it matters most
\end{enumerate}

\vspace{0.3em}
{\small \textbf{The punchline has shifted:} It's not ``VAE is 3.4$\times$ better.'' It's ``linear methods are anti-correlated with ground truth, and dimensionality predicts where.''}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[standout]
  ...
  Linear methods are \emph{anti-correlated} with causal importance.\\[0.3em]
  Dimensionality tells you which tool to trust.\\[0.3em]
  {\small (But check your metrics against external ground truth.)}
\end{frame}
