...
The preceding analyses identify the evidence subspace via LDA---a linear, supervised method that maximizes class separation. But class separation is a correlational objective: LDA finds directions along which evidence categories are most distinguishable, not necessarily directions along which interventions most affect the downstream choice. We ask whether a nonlinear method with an explicit causal inductive bias can find a stronger causal subspace, and whether the improvement reveals structure that LDA misses.

\textbf{Structured VAE architecture.} We train a structured variational autoencoder \citep{narayanaswamy2017learning} per region. The architecture enforces an explicit factorization of the latent space into a $k$-dimensional ``choice'' subspace and an $m$-dimensional ``other'' subspace ($m = 15$). Only the choice latent is trained to predict the animal's left/right decision (via an auxiliary classification loss with weight $\alpha = 10$), while both latent groups jointly reconstruct the population activity. This design encourages the choice subspace to capture the minimal sufficient representation for the decision while the other subspace absorbs remaining variance. The model trains for 300 epochs per region across all available sessions ($n = 39$).

\textbf{Choice subspace dimensionality ablation.} We vary the choice subspace dimensionality $k \in \{1, 2, 3\}$ to test sensitivity to this hyperparameter. Table~\ref{tab:vae_k_ablation} shows that IIA is stable across all three settings: even $k = 1$ achieves a 3.7$\times$ improvement over LDA and wins 72/73 regions. The causal evidence-to-choice signal is effectively one-dimensional.
...
\textbf{VAE subspaces are dramatically more causal than LDA.} We measure IIA on the VAE's choice subspace using the same swap protocol as the LDA analysis: for each region, swap the choice-subspace projection between opposite-evidence trial pairs and measure how often the choice classifier flips. The VAE subspace produces mean IIA $= 0.56$ (SD $0.08$) vs.\ LDA mean IIA $= 0.17$ (SD $0.09$)---a 3.4$\times$ improvement, with Cohen's $d = 4.5$. The VAE wins in 73/73 regions (Wilcoxon signed-rank $W = 2701$, $p = 5.7 \times 10^{-14}$), with every region showing at least a $+0.06$ improvement and some showing $+0.80$ (CA). The VAE also exceeds the random-direction null in every region (73/73; mean null IIA $= 0.23$, mean improvement over null $= 0.33$, minimum improvement $= 0.13$; Mann-Whitney $p = 1.4 \times 10^{-25}$).

The improvement is not uniform across the brain. The median IIA is $0.57$ (IQR: $0.52$--$0.62$), with 78\% of regions (57/73) exceeding $0.50$---meaning interventions on the VAE choice subspace flip the predicted choice more often than not in the majority of brain regions. The top regions include CA ($0.80$), CA2 ($0.71$), SSs ($0.68$), MG ($0.67$), and ILA ($0.67$). Even the weakest region (CL, $0.32$) substantially exceeds the random null.

\textbf{Low-dimensional regions benefit most.} We split regions at the median power-law exponent ($\alpha = 6.2$). Low-$\alpha$ (high-dimensional) regions show a larger VAE improvement (mean $\Delta$IIA $= 0.46$) than high-$\alpha$ (low-dimensional) regions (mean $\Delta$IIA $= 0.33$), despite high-dimensional regions having lower LDA IIA to begin with ($0.13$ vs.\ $0.20$). This is consistent with the interpretation that high-dimensional regions encode choice information in nonlinear subspaces that LDA's linear projections miss entirely. The VAE, unconstrained by linearity, recovers this signal. Furthermore, VAE IIA anti-correlates with $\alpha$ ($\rho = -0.48$, $p = 1.5 \times 10^{-5}$): high-dimensional regions have the highest VAE-based causal signal, reversing the pattern seen with LDA.

\textbf{VAE finds genuinely different subspaces.} The VAE and LDA subspaces are nearly orthogonal: mean Grassmannian geodesic distance $= 2.34$ (SD $0.15$), where $\pi/2 \approx 1.57$ per dimension. This means the VAE does not simply refine or denoise the LDA solution---it finds a fundamentally different set of directions in population activity space that are far more causally effective. The large Grassmannian distance combined with the large IIA improvement implies the existence of a causal choice pathway that is invisible to linear discriminant methods.

\textbf{Model confidence tracks causal effectiveness.} The VAE's posterior uncertainty (entropy of the approximate posterior over the choice latent) anti-correlates with IIA ($\rho = -0.26$, $p = 0.028$): regions where the model is most confident about the identity of the choice subspace are precisely those where intervening on that subspace most strongly affects the downstream decision. This provides an internal consistency check---the model is not merely memorizing choice labels but learning a representation whose quality is reflected in its own uncertainty estimates.

\textbf{Regional breakdown.} Grouping by anatomical category reveals that hippocampal regions (CA, CA2, CA3, DG, SUB; mean VAE IIA $= 0.60$) and prefrontal regions (PL, ILA, ORB, ACA; mean VAE IIA $= 0.61$) show the strongest causal signal, followed by sensory cortex (VIS*, AUD, SS*; mean $= 0.58$) and motor cortex (MOs, MOp; mean $= 0.54$). The hippocampal finding is notable: these regions showed weak LDA-based IIA ($0.13$), suggesting the evidence-to-choice pathway in hippocampus operates through a nonlinear subspace that linear methods systematically underestimate.

These results have three implications. First, the causal evidence-to-choice pathway exists broadly across the brain but is substantially underestimated by linear subspace methods---the ``true'' causal signal is 3.4$\times$ stronger than what LDA reveals. Second, the choice of subspace identification method is not a neutral methodological decision: it determines not just the magnitude but the regional pattern of the measured causal effect, reinforcing the paper's central claim that neural geometry is not metric-neutral. Third, the anti-correlation between dimensionality and VAE IIA suggests that high-dimensional regions, far from being causally weak, implement the strongest evidence-to-choice pathways through nonlinear subspaces that require appropriate methods to detect.
