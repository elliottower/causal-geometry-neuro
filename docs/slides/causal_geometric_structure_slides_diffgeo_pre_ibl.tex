...
\subtitle{The differential geometry of neural population codes}
\author{Elliot Tower}
\date{2026}
\institute{}
...
\textbf{Part 1: Core geometric findings}
{\small
\begin{enumerate}
...
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Part 2: Deeper geometric analysis}
{\small
\begin{enumerate}
  \setcounter{enumi}{3}
  \item Neural manifold framework (potent/null)
  \item Multi-task generalization (Grassmannian)
  \item Grassmannian dissimilarity matrices
\end{enumerate}
}
\end{column}
\end{columns}
...
\begin{itemize}
  \item \textbf{One dataset} (Steinmetz 2019, 10 mice)
  \item \textbf{10 mice}: too few to quantify individual differences
...
\begin{itemize}
  \item \textbf{IBL replication}: same pipeline, independent dataset---does $\alpha$ predict dissociation there too?
...
\begin{enumerate}
  \item CKA and Procrustes anti-correlate ($\rho = -0.85$); dimensionality explains why
  \item Variance structure is universal ($0.94$); neural content is not ($0.09$)
...
\vspace{0.5em}
{\small \textbf{Takeaway:} Dimensionality mediates tool disagreement, potent/null structure explains why, and Grassmannian geometry captures the underlying functional organization.}
\end{frame}
